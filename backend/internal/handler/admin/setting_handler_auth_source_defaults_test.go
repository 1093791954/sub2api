...
func TestSettingHandler_UpdateSettings_AcceptsUnlimitedDefaultConcurrency(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPut, "/api/v1/admin/settings", bytes.NewBufferString(`{"default_concurrency":0}`))
	c.Request.Header.Set("Content-Type", "application/json")

	handler.UpdateSettings(c)

	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "0", repo.values[service.SettingKeyDefaultConcurrency])
}
