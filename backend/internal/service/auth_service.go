...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	accountMode := s.settingService.IsAccountLoginEnabled(ctx)
	if accountMode {
		var err error
	...
	if err := s.createUserAndClaimInvitation(ctx, user, invitationRedeemCode, accountMode); err != nil {
...
func (s *AuthService) createUserAndClaimInvitation(ctx context.Context, user *User, invitation *RedeemCode, accountMode bool) error {
	if s == nil || s.userRepo == nil {
		return ErrServiceUnavailable
	}
	createUser := s.createUserWithRegistrationEmailGuard
	if accountMode {
		createUser = s.userRepo.Create
	}
	commitUser := func(execCtx context.Context) error {
		if err := createUser(execCtx, user); err != nil {
			return err
		}
		if invitation == nil {
			return nil
