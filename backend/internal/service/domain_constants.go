...
// KeyLoginSyntheticEmailDomain is the reserved email domain for access-key based users.
const KeyLoginSyntheticEmailDomain = "@key.local"
...
const (
	...
	SettingKeyAccountLoginEnabled              = "account_login_enabled"               // 是否使用账号密码注册登录（复用 users.email 作为唯一标识）
	...
	SettingKeyKeyRegisterSecret                = "key_register_secret"                 // 密钥注册所需的管理员验证码（空表示禁用密钥注册）
