...
- **访问密钥认证** - 用户无需注册邮箱，凭一个 32–64 位的访问密钥即可完成注册和登录。管理员在系统设置中配置注册密钥，以控制密钥注册的开放程度。

### 访问密钥认证

用户无需邮箱/密码，使用访问密钥（32–64 位字母数字字符串）即可注册和登录：

- **注册**：`POST /api/v1/auth/key-register`，请求体：`{ "access_key": "...", "admin_secret": "..." }`。`admin_secret` 需与管理员在系统设置中配置的「密钥注册码」匹配。
- **登录**：`POST /api/v1/auth/key-login`，请求体：`{ "access_key": "..." }`

管理员在**管理员后台 → 系统设置**中设置非空的「密钥注册码」来开启此功能。
